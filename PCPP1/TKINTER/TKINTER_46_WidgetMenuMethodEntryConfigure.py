"""
Note: you cannot modify any of the (sub)menu item by using the standard
config() method invocation, because from tkinter's point of view, the item is
not a widget – it’s only a very specific widget component.

If you want to manipulate a menu’s item, you should use a dedicated method
named entryconfigure(). The method accepts two parameters:
...
The snippet shows you how it works – it plays with the second entry’s
state property
...
tearoff     	set to zero removes the tear-off decoration from the top of the
                cascade
state	        when set to DISABLED, the menu item is grayed and inaccessible;
                setting it to ACTIVE restores its normal functionality
...
entryconfigure(i, prop=val,...)	modifies the i-th menu item’s property
                                named prop
"""
