"""
If you want some of the (sub)menu items to be accessible through a dedicated
hot-key, you have to perform two steps:

* set the item’s property named accelerator with a string naming the hot-key
(note: this has no other effect than just displaying the right-aligned string
inside the item – no callback is set at the moment)
* make a global binding linking the hot-key with a proper callback.

Look – we’ve applied these steps to our code making Ctrl-Q a shortcut designed
to close the application – it happens in lines #24 (we modified the
are_you_sure() function header), #48 (we added the accelerator property) and
#53 (global binding to <Control-q>).
"""
